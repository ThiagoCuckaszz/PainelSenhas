...
db.serialize(function () {
  db.run(
    "CREATE TABLE IF NOT EXISTS fichas (id INTEGER PRIMARY KEY AUTOINCREMENT, tipo TEXT, numero INT, data DATETIME, hora DATETIME, atendido Boolean DEFAULT FALSE)"
  );
});
...
function obterUltimaFichaImprimida() {
  db.all(
    "SELECT tipo, MAX(numero) as lastFicha FROM fichas WHERE data = ? GROUP BY tipo",
    [dataAtual],
    function (err, rows) {
      if (!err && rows) {
        console.log(rows);
...
function gerarFicha(tipoFicha, res) {
  ...
  try {
    ...
    db.run(
      "INSERT INTO fichas (tipo, numero, data, hora) VALUES (?, ?, ?,?)",
      tipoFicha,
      numeroFichas[tipoFicha],
      dataAtual,
      horaAtual
...
function atender(tipoFicha, res) {
  db.all(
    'SELECT id, Min(numero) as lastFicha FROM fichas WHERE data = ? AND atendido = "0" and tipo = ? GROUP BY tipo',
    [dataAtual, tipoFicha],
    function (err, fichaChamar) {
      if (err) {
        console.error(err);
      ...
      if (fichaChamar.length > 0) {
        ...
        db.run(
          "UPDATE fichas SET atendido = 1 WHERE id = ?",
          [id_update],
          function (err) {
            if (err) {
              ...
              return res
                .status(500)
                .send("Erro ao atualizar ficha no banco de dados");
            }
        ...
        console.error(
          "Nenhuma ficha encontrada para os critérios especificados."
        );
        res
          .status(404)
          .send("Nenhuma ficha encontrada para os critérios especificados.");
      }
    }
  );
}
